...
- Fix plan:
  - Add src/utils/testDatabase.ts exporting runAllTests (even as a no-op or basic connectivity check), or refactor SettingsScreen to lazy-import under **DEV** with try/catch.
...
- Fix plan:
  - Condition on **DEV** or a feature flag before merging sampleConfessions.
...
- Fix plan:
  - Add src/utils/testDatabase.ts exporting runAllTests (no-op or basic DB connectivity checks) or guard import with **DEV** and dynamic import.
...
22. Excessive console logging without **DEV** guards
...
- Fix plan:
  - Wrap non-critical logs in if (**DEV**) conditions.
...
- Affected file:
  - src/screens/SettingsScreen.tsx
- Issue:
  - Test database button shown only in **DEV** – good. But runAllTests should also guard heavy operations and not block UI.
- Fix plan:
...
- Fix plan:
  - Gate demo implementations behind a feature flag or **DEV** to prevent confusion in production builds.
...
- Issue:
  - Extensive console.log output is helpful during development but should be gated behind **DEV** to reduce noise and potential perf impact.
- Fix plan:
  - Wrap verbose logs with if (**DEV**) guards.
...
- Fix plan:
  - Gate sampleConfessions usage behind **DEV** and/or a feature flag.
...
- Logging
  - Excessive logs — gate with **DEV** to reduce overhead.
...
- Create src/utils/testDatabase.ts that exports async function runAllTests(): Promise<void>
  ...
  - Guard heavy operations behind **DEV** to avoid shipping test pathways.

4. Gate logs under **DEV**

- Wrap verbose console.log in AppNavigator and authStore with if (**DEV**).

5. Gate sampleConfessions under **DEV**
...
- Guard logs and remove fonts object from theme
  Pattern: Wrap console.log(...) statements with if (**DEV**) { ... }
  Example:
  // Before
...
- Guard verbose logs under **DEV**
  Pattern changes within checkAuthState and listeners:
  console.log(...) => if (**DEV**) { console.log(...) }
...
- Fix plan:
  - Rename envs to EXPO*PUBLIC_SUPABASE_URL and EXPO_PUBLIC_SUPABASE_ANON_KEY (or EXPO_PUBLIC_TOXICCONFESSIONS_SUPABASE*\* if you prefer namespacing) and update usage.
...
- Developer utility
  - Added src/utils/testDatabase.ts with lightweight read-only connectivity checks used by SettingsScreen in **DEV**.
...
- Optional: wrap noisy console logs in **DEV** in confessionStore and AppNavigator.
...
- Log gating
  - Wrapped AppNavigator console logs in **DEV** to reduce noise in production.
...
- Supabase env guard and naming alignment
  - src/lib/supabase.ts: support EXPO*PUBLIC_SUPABASE_URL/ANON_KEY with fallback to legacy \*\_VIBECODE*\* vars; clearer dev error message.
...
- Log gating in stores
  - src/state/confessionStore.ts: wrapped noisy console.log calls in **DEV**.
...
3. **End-to-end smoke test implementation**
   - src/utils/**tests**/videoSmokeTest.ts: Complete test suite for video pipeline
  ...
  while (true) {
    ...
    for (const it of items) {
      ...
      const { error: upErr } = await supabase.storage
        .from("confessions")
        .upload(srcPath, file, { upsert: true, contentType: it.metadata?.mimetype });
