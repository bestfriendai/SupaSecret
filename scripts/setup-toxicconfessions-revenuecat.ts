...
async function main() {
  ...
  try {
    ...
    const oldEntitlement = existingEntitlements.find(
      (e: any) => e.lookup_key === "supasecret_plus" || e.id === "supasecret_plus",
    );
    if (oldEntitlement) {
    ...
    let entitlementId = existingEntitlements.find((e: any) => e.lookup_key === "toxicconfessions_plus")?.id;
    ...
    const productIds = ["toxicconfessions_plus_monthly", "toxicconfessions_plus_annual"];
    ...
    let offeringId =
      existingOfferings.find((o: any) => o.lookup_key === "default" || o.id === "default")?.id || "default";
