...
export async function runVisionCameraDiagnostics(): Promise<DiagnosticResult[]> {
  ...
  // Test 4: react-native-worklets-core
  try {
    const worklets = await import('react-native-worklets-core');
    const exports = Object.keys(worklets);
    ...
    console.log('✅ react-native-worklets-core loaded');
    console.log('   Exports:', exports.join(', '));
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    results.push({
    ...
    console.error('❌ react-native-worklets-core failed to load:', errorMessage);
  }
