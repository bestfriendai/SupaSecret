...
export const useOptimizedReplies = () => {
  ...
  const loadRepliesForVisibleItems = useCallback(
    async (visibleIds: string[]) => {
      // Filter out already loaded, currently loading items, or sample data
      const newIds = visibleIds.filter(
        (id) => !loadedReplies.has(id) && !loadingPromises.current.has(id) && isValidForDatabase(id), // Skip sample data with invalid UUIDs
      );
  ...
  const preloadRepliesForConfession = useCallback(
