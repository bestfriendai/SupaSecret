...
const GeometricAvatar = memo(({ seed, size = 40 }: { seed: string; size?: number }) => {
  const generatePattern = useCallback((seed: string) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {level > 0 && (
            <View className="absolute -left-3 top-0 bottom-0 w-0.5 bg-purple-500/20 rounded-full" />
          )}
          ...
          <View className="flex-1 ml-3">
            <View className="flex-row items-center mb-1">
              ...
              {item.edited_at && (
                <Text className="text-gray-500 text-xs ml-1 italic">
                  (edited)
                </Text>
              )}
            </View>
            ...
            {item.reactionCounts && Object.keys(item.reactionCounts).length > 0 && (
              <View className="flex-row mt-2 gap-1">
                {Object.entries(item.reactionCounts || {})
                  ...
                  <View
                    key={type}
                    className="flex-row items-center bg-purple-500/10 px-2 py-1 rounded-full"
                  >
            ...
            <View className="flex-row mt-2 gap-3">
              ...
              {hasReplies && (
                <TouchableOpacity
                  onPress={() => onToggleThread(item.id, item)}
                  className="flex-row items-center"
                >
                  <Ionicons
...
const EnhancedCommentBottomSheet = React.memo(
  ...
  const typingUsers = useMemo(() => new Map(typingUsersArr.map(t => [t.replyId || 'root', true])), [typingUsersArr]);
  ...
  const listData = isSearching && searchQuery.trim() ? searchResults : replies;
    ...
    const handleReaction = useCallback(async (item: any, type: ReactionType) => {
    ...
    const toggleThread = useCallback(async (id: string, item: any) => {
          ...
          {(['recent', 'popular', 'all'] as const).map(filter => (
            <TouchableOpacity
              ...
              className={`px-3 py-1.5 rounded-full ${
                selectedFilter === filter ? 'bg-purple-500' : 'bg-gray-800'
              }`}
            >
              <Text
