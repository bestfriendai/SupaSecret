...
const defaultMessages = {
  camera: 'Camera access is required to record videos. Your privacy is important to us - we only access the camera when you\'re actively recording.',
  microphone: 'Microphone access is needed to capture audio with your videos. Audio is processed locally for privacy.',
  mediaLibrary: 'Media library access allows you to save and share your videos. We only access files you explicitly choose.',
...
export const PermissionGate: React.FC<PermissionGateProps> = ({
  ...
  const requestPermission = async (type: 'camera' | 'microphone' | 'mediaLibrary') => {
    ...
    try {
      ...
      if (result) {
        ...
        setError(`${type.charAt(0).toUpperCase() + type.slice(1)} permission was denied. You can enable it in settings.`);
      }
    } catch (err) {
      const standardError: StandardError = {
  ...
  if (error) {
    return (
      <ErrorState
        message={error}
        onRetry={checkInitialPermissions}
        type="permission"
      />
    );
  }
    ...
    <View style={styles.container}>
      <View style={styles.content}>
        ...
        <Text style={styles.title}>
          {permanentlyDenied ? 'Permissions Required' : 'Grant Permissions'}
        </Text>
                  ...
                  <View style={styles.permissionInfo}>
                    ...
                    <View style={styles.permissionTextContainer}>
                      ...
                      <Text style={styles.permissionDescription}>
                        {messages[permission as keyof typeof messages]}
                      </Text>
                    </View>
                  </View>
                  <TouchableOpacity
                    style={styles.grantButton}
                    onPress={() => requestPermission(permission as any)}
                  >
