  ...
  const handleSetupReports = async () => {
    ...
    try {
      ...
      if (tableExists) {
        Alert.alert(
          'Already Set Up!',
          'The reports table already exists and is working correctly.',
          [{ text: 'OK' }]
        );
        setStatus('success');
        setIsLoading(false);
  ...
  const handleTestReports = async () => {
    ...
    try {
      ...
      if (success) {
        ...
        Alert.alert(
          'Test Failed',
          'The reports system is not working. Check the console for details.',
          [{ text: 'OK' }]
        );
      }
      ...
      Alert.alert(
        'Test Error',
        'An error occurred during testing. Check the console for details.',
        [{ text: 'OK' }]
      );
    } finally {
      setIsLoading(false);
  ...
  return (
    <View className="bg-gray-900 border border-gray-700 rounded-lg p-4 m-4">
      <View className="flex-row items-center mb-3">
        ...
        <Text className="text-white text-16 font-semibold ml-2">
          Reports Migration Helper
        </Text>
      </View>
      ...
      <View className="space-y-3">
        ...
        <Pressable
          className={`rounded-lg py-3 px-4 border border-gray-600 ${
            isLoading ? 'bg-gray-800' : 'bg-gray-800'
          }`}
      ...
      {status === 'error' && (
        <View className="mt-3 p-3 bg-red-900/20 border border-red-700 rounded-lg">
          <Text className="text-red-400 text-13">
            Manual setup required. Check console for details.
          </Text>
        </View>
      )}
      ...
      {status === 'success' && (
        <View className="mt-3 p-3 bg-green-900/20 border border-green-700 rounded-lg">
          <Text className="text-green-400 text-13">
            ✅ Reports system is ready! You can now remove this component.
          </Text>
        </View>
      )}
    </View>
