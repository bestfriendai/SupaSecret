    ...
    <SafeAreaView className="flex-1 bg-gray-900">
      <View className="flex-row items-center justify-between p-4 border-b border-gray-700">
        ...
        <View className="flex-row items-center space-x-2">
          <Pressable className="bg-blue-500 rounded-full px-4 py-2" onPress={runBuildTest} disabled={isRunning}>
            <Text className="text-white font-medium">{isRunning ? "Testing..." : "Run Test"}</Text>
          </Pressable>
          <Pressable className="bg-gray-600 rounded-full px-4 py-2" onPress={clearLogs}>
      ...
      <ScrollView className="flex-1 p-4">
        <View className="bg-gray-800 rounded-lg p-4 mb-4">
          ...
          <View className="space-y-1">
            ...
            <Text className="text-white">Native Capable: {env.isNativeCapable ? "✅ Yes" : "❌ No"}</Text>
            <Text className="text-white">FFmpeg Ready: {env.ffmpegReady ? "✅ Yes" : "⏳ Pending"}</Text>
          </View>
        </View>
        ...
        <View className="bg-gray-800 rounded-lg p-4">
          ...
          {testResults.map((result, index) => (
            <View key={index} className="mb-1">
              <Text className="text-gray-300 text-sm font-mono leading-5">{result}</Text>
            </View>
          ))}
        </View>
      </ScrollView>
