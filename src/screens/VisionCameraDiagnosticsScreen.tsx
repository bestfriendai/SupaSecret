      ...
      <ScrollView style={styles.content}>
        ...
        {hasRun && (
          <View style={[styles.summaryCard, allLoaded ? styles.successCard : styles.errorCard]}>
            ...
            />
            <Text style={styles.summaryTitle}>{allLoaded ? "All Modules Loaded" : "Some Modules Failed"}</Text>
              ...
              {result.loaded ? (
                <View style={styles.resultDetails}>
                  ...
                  {result.exports && result.exports.length > 0 && (
                    <>
                      <Text style={[styles.detailLabel, { marginTop: 8 }]}>Exports ({result.exports.length}):</Text>
                      <Text style={styles.detailValue}>{result.exports.join(", ")}</Text>
                    </>
                  )}
                </View>
              ) : (
                <View style={styles.resultDetails}>
                  <Text style={styles.detailLabel}>Status:</Text>
                  <Text style={[styles.detailValue, styles.errorText]}>❌ Failed to load</Text>

                  {result.error && (
                    <>
                      <Text style={[styles.detailLabel, { marginTop: 8 }]}>Error:</Text>
                      <Text style={[styles.detailValue, styles.errorText]}>{result.error}</Text>
                    </>
                  )}
                </View>
              )}
        ...
        {hasRun && !allLoaded && (
          <View style={styles.instructionsCard}>
            ...
            <Text style={styles.instructionText}>1. Make sure you're running a development build (not Expo Go)</Text>
            <Text style={styles.instructionText}>2. Clean and rebuild the app:</Text>
            <Text style={styles.codeText}>npx expo run:ios --clean</Text>
            <Text style={styles.instructionText}>3. Check that all dependencies are installed:</Text>
            <Text style={styles.codeText}>npm install</Text>
            <Text style={styles.instructionText}>4. For iOS, reinstall pods:</Text>
            <Text style={styles.codeText}>cd ios && pod install && cd ..</Text>
          </View>
        )}

        {/* Rerun Button */}
        <Pressable style={styles.rerunButton} onPress={runDiagnostics} disabled={loading}>
